...
export default function DispatchBoard() {
  ...
  const grid: React.CSSProperties = {
    position: 'fixed', inset: 0, backgroundImage: 'linear-gradient(rgba(120,190,32,0.03) 1px,transparent 1px),linear-gradient(90deg,rgba(120,190,32,0.03) 1px,transparent 1px)',
    backgroundSize: '48px 48px', pointerEvents: 'none', zIndex: 0,
  };
  const topBar: React.CSSProperties = {
    position: 'sticky', top: 0, zIndex: 10, background: 'rgba(27,27,75,0.85)', backdropFilter: 'blur(20px)',
  ...
  const content: React.CSSProperties = { position: 'relative', zIndex: 1, maxWidth: '1000px', margin: '0 auto', padding: '28px 20px' };
  ...
  const statusBadge = (status: string): React.CSSProperties => ({
    ...
    color: status === 'COMPLETED' ? '#78BE20' : '#e8a800', border: `1px solid ${status === 'COMPLETED' ? 'rgba(120,190,32,0.3)' : 'rgba(255,185,0,0.25)'}`,
  });
