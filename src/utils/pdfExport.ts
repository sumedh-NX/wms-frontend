...
// ═══════════════════════════════════════════════════════
// NHK SPRINGS PDF EXPORT
// ═══════════════════════════════════════════════════════
export function exportNhkPDF(dispatch: any, logs: any[], bins: any[], parts: any[]): void {
  const pdf = new jsPDF('p', 'mm', 'a4');
  let y = drawReportHeader(pdf, dispatch, logs, 'NHK SPRINGS');
  ...
  const totalParts    = parts.length;
  const partsPerBin   = dispatch?.ref_case_pack ?? 5;
  const expectedTotal = dispatch?.total_schedule_bins
    ? dispatch.total_schedule_bins * partsPerBin : '—';
  ...
  if (bins.length === 0) {
    ...
    bins.forEach((bin, binIdx) => {
      const binParts    = parts.filter((p: any) => p.bin_id === bin.id);
      const binComplete = binParts.length >= partsPerBin;
      y = checkPage(pdf, y, 14);
      ...
      pdf.setFontSize(7.5); pdf.setFont('helvetica', 'bold'); pdf.setTextColor(...C.DARK);
      pdf.text(
        `Bin ${binIdx + 1}: ${trunc(safe(bin.bin_number), 18)}  |  Product: ${safe(bin.product_code)}  |  Parts: ${binParts.length}/${partsPerBin}  |  Date: ${safe(bin.supply_date)}  |  ${binComplete ? 'COMPLETE' : 'INCOMPLETE'}`,
        MARGIN + 2, y + 5.5
      );
      y += 10;
      ...
      if (binParts.length === 0) {
        ...
        const partColW = COL_W / 2;
        let colIdx = 0, rowY = y;
        binParts.forEach((part: any, pIdx: number) => {
          if (colIdx === 0) { y = checkPage(pdf, rowY, 6); rowY = y; }
          if (pIdx % 4 === 0 && pIdx > 0) {
            pdf.setFillColor(...C.ROW_ALT);
            pdf.rect(MARGIN, rowY, COL_W, 5.5, 'F');
          }
          pdf.setFontSize(6.5); pdf.setFont('helvetica', 'normal'); pdf.setTextColor(...C.DARK);
          pdf.text(`${pIdx + 1}. ${trunc(safe(part.part_code), 38)}`, MARGIN + colIdx * partColW + 2, rowY + 4);
          colIdx++;
          if (colIdx === 2) { colIdx = 0; rowY += 5.5; }
        });
        if (colIdx !== 0) rowY += 5.5;
        y = rowY + 3;
      }
  ...
  y = drawSignatureBoxes(pdf, y);
  footer(pdf);
  pdf.save(`NHK_Dispatch_${dispatch?.dispatch_number}.pdf`);
}
